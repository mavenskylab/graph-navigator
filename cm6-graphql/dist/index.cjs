...
const schemaEffect = state.StateEffect.define();
const schemaStateField = state.StateField.define({
    create() { },
    update(schema, tr) {
...
const optionsEffect = state.StateEffect.define();
const optionsStateField = state.StateField.define({
    create() { },
    update(opts, tr) {
        for (const e of tr.effects) {
...
const fillAllFieldsCommands = (view) => {
    ...
    const token = graphqlLanguageService.getTokenAtPosition(view.state.doc.toString(), pos);
...
const showInDocsCommand = (view) => {
    ...
    if (schema && (opts === null || opts === void 0 ? void 0 : opts.onShowInDocs)) {
        const tInfo = graphqlLanguageService.getTypeInfo(schema, token.state);
...
const punctuations = '( ) { } : [ ]';
const graphqlLanguage = language.LRLanguage.define({
    parser: parser.configure({
        props: [
            highlight.styleTags({
                Variable: highlight.tags.variableName,
                BooleanValue: highlight.tags.bool,
                Description: highlight.tags.string,
                StringValue: highlight.tags.string,
                Comment: highlight.tags.lineComment,
                IntValue: highlight.tags.integer,
                FloatValue: highlight.tags.float,
                EnumValue: highlight.tags.special(highlight.tags.name),
                NullValue: highlight.tags.null,
                DirectiveName: highlight.tags.modifier,
                [keywords]: highlight.tags.keyword,
                OperationType: highlight.tags.definitionKeyword,
                FieldName: highlight.tags.propertyName,
                Field: highlight.tags.propertyName,
                ArgumentAttributeName: highlight.tags.attributeName,
                Name: highlight.tags.atom,
                '( )': highlight.tags.paren,
                '{ }': highlight.tags.brace,
                ',': highlight.tags.separator,
                [punctuations]: highlight.tags.punctuation,
            }),
            // https://codemirror.net/docs/ref/#language.indentNodeProp
            language.indentNodeProp.add({
                [nodesWithBraces]: language.delimitedIndent({ closing: '}', align: true }),
            }),
            language.foldNodeProp.add({
                [nodesWithBraces]: language.foldInside,
            }),
        ],
    }),
    languageData: {
...
function graphqlLanguageSupport() {
    return new language.LanguageSupport(graphqlLanguage);
}

const AUTOCOMPLETE_CHARS = /^[a-zA-Z0-9_@(]$/;
const completion = graphqlLanguage.data.of({
    autocomplete(ctx) {
        ...
        const results = graphqlLanguageService.getAutocompleteSuggestions(schema, val, pos, undefined, undefined, opts === null || opts === void 0 ? void 0 : opts.autocompleteOptions);
        if (results.length === 0) {
            return null;
        }
        return {
...
const jump = view.EditorView.domEventHandlers({
    click(evt, view) {
        var _a, _b, _c;
        const schema = getSchema(view.state);
        if (!schema) {
...
const lint = lint$1.linter(view => {
    const schema = getSchema(view.state);
    const options = getOpts(view.state);
    if (!schema) {
        return [];
    }
    const validationErrors = graphql$1.validateSchema(schema);
    ...
    const results = graphqlLanguageService.getDiagnostics(view.state.doc.toString(), schema);
    return results
        .map((item) => {
...
exports.fillAllFieldsCommands = fillAllFieldsCommands;
exports.getOpts = getOpts;
exports.getSchema = getSchema;
exports.graphql = graphql;
exports.graphqlLanguage = graphqlLanguage;
exports.graphqlLanguageSupport = graphqlLanguageSupport;
exports.isMetaKeyPressed = isMetaKeyPressed;
exports.jump = jump;
exports.lint = lint;
exports.offsetToPos = offsetToPos;
exports.optionsStateField = optionsStateField;
exports.posToOffset = posToOffset;
exports.schemaStateField = schemaStateField;
exports.showInDocsCommand = showInDocsCommand;
exports.stateExtensions = stateExtensions;
exports.updateOpts = updateOpts;
